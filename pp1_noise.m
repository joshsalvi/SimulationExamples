% pp1_noise.m explores stability for Euler's method in a simple predator prey model without noise

clf

sigma = 2;                           % strength of the noise
...
for p=1:3,
  ...
  for i=1:N                          % start taking steps
    x(i+1)=x(i)+0.2*(y(i)-50*x(i))*h+sigma*sqrt(h)*randn;
    y(i+1)=y(i)+4*(20-x(i))*y(i)*h+sigma*sqrt(50*h)*randn;
  end;
