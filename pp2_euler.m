...
for p=1:3,
  ...
  for i=1:N                          % start taking steps
    x(i+1)=x(i)+0.1*(y(i)-1000)*x(i)*h;
    y(i+1)=y(i)+2*(20-x(i))*y(i)*h;
  end;
...
subplot(3,2,5);
  ...
  ymin = 800;
  ymax = 1200;
  [xg,yg] = meshgrid(xmin:0.4:xmax,ymin:13:ymax);    % prepare points on a grid
  px = 0.1*(yg - 1000) .* xg;                  % rate of growth of wolf population
  py = 2*(20-xg) .* yg;                      % rate of growth of deer population
